...
cp -R build/dependencyUpdates/. deploy-stuff/Deathmatch
cp -R build/docs/javadoc/. deploy-stuff/Deathmatch/javadoc
cp -R build/reports/. deploy-stuff/Deathmatch
cp -R build/libs/. deploy-stuff/Deathmatch

# create mvn repo
mkdir deploy-stuff/mvn-repo/
mvn deploy:deploy-file -Dfile=build/libs/Deathmatch-1.0-SNAPSHOT.jar -DpomFile=pom.xml  -Durl=file://${TRAVIS_BUILD_DIR}/deploy-stuff/mvn-repo
...
git add .
echo "commit"
git commit -m "Deploy to Github Pages (Deathmatch)"
echo "push"
